    ...
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView>
        ...
        <View style={s.section}>
          ...
          <Text style={s.payMethod}>Thanh toán: {order.paymentMethod === 'cod' ? '💵 Tiền mặt' : order.paymentMethod === 'wallet' ? '💳 Ví CRABOR' : '🏦 Chuyển khoản'}</Text>
        </View>
...
export function VouchersScreen({ navigation }) {
  ...
  const SAMPLE_CODES = ['CRABOR10', 'FREESHIP', 'NEWUSER50', 'SALE20'];
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView contentContainerStyle={{ padding: Spacing.lg }}>
        <Text style={s.sectionTitle}>Kiểm tra mã voucher</Text>
        <View style={{ flexDirection: 'row', gap: 8, marginBottom: 12 }}>
          <TextInput style={[s.fieldInput, { flex: 1, marginBottom: 0 }]} placeholder="Nhập mã voucher..." value={code} onChangeText={setCode} autoCapitalize="characters" />
        ...
        {result && (
          result.ok
            ? <View style={s.voucherResult}><Text style={s.voucherResultOk}>✅ Hợp lệ! Giảm {formatCurrency(result.data?.discount || 0)}</Text></View>
            : <View style={[s.voucherResult, { backgroundColor: '#FFF0F0' }]}><Text style={{ color: Colors.danger, fontWeight: '700' }}>❌ {result.msg}</Text></View>
        )}
        <Text style={[s.sectionTitle, { marginTop: 24, marginBottom: 12 }]}>Mã thường dùng</Text>
        {SAMPLE_CODES.map(c => (
          <TouchableOpacity key={c} style={s.voucherChip} onPress={() => setCode(c)}>
            <Text style={s.voucherChipCode}>{c}</Text>
            <Text style={s.voucherChipArrow}>→ Thử</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
    </SafeAreaView>
