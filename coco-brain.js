...
function buildSystemPrompt(opts = {}) {
  ...
  // Inject thông tin tài khoản thật của user
  if (userContext && Object.keys(userContext).length > 0) {
    ...
    if (userContext.activeLoan)
      lines.push(`• Đang vay: ${(userContext.activeLoan.amount || 0).toLocaleString('vi-VN')}đ (${userContext.activeLoan.status})`);

    // Dữ liệu shipper
    if (userContext.userType === 'shipper') {
      if (userContext.tier)         lines.push(`• Hạng: ${userContext.tier}`);
      ...
      if (userContext.earnings)     lines.push(`• Ví shipper: ${userContext.earnings.toLocaleString('vi-VN')}đ`);
    }

    // Dữ liệu partner
    if (userContext.userType === 'partner') {
      if (userContext.bizName)      lines.push(`• Tên cửa hàng: ${userContext.bizName}`);
      if (userContext.activeOrders !== undefined)
        lines.push(`• Đơn đang xử lý: ${userContext.activeOrders}`);
      if (userContext.partnerWallet)
        lines.push(`• Ví đối tác: ${userContext.partnerWallet.toLocaleString('vi-VN')}đ`);
    }
