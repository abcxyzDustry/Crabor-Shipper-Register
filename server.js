...
// GET /api/admin/wallet-queue — Xem danh sách pending
app.get("/api/admin/wallet-queue", async (req, res) => {
  try {
    if (!req.session.adminId) return res.status(401).json({ success: false });
    const { status = "pending", page = 1, limit = 50 } = req.query;
    const queue = await WalletQueue.find({ status })
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(parseInt(limit));
    const total = await WalletQueue.countDocuments({ status });
...
// GET /api/admin/wallet-queue/stats — Thống kê pending
app.get("/api/admin/wallet-queue/stats", async (req, res) => {
  try {
    if (!req.session.adminId) return res.status(401).json({ success: false });
    const [pending, approved, totalPendingAmt] = await Promise.all([
