...
app.post("/api/shipper/register", async (req, res) => {
  try {
    ...
    if (payload.refCode) {
      const agent = await Sales.findOne({ refCode: payload.refCode.toUpperCase(), status: "active" });
      if (agent) {
        await Referral.create({
          salesId: agent._id, refCode: agent.refCode,
          targetType: "shipper", targetId: shipper._id,
          targetPhone: phone, targetName: shipper.fullName,
          module: "shipper"
        });
      }
    }
...
app.post("/api/partner/register", async (req, res) => {
  try {
    ...
    const base = { phone, firstName, lastName, email, address, district, refCode: refCode?.toUpperCase() };
    let data = { ...base };
