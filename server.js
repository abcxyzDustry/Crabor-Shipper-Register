...
// ── OTP (dùng chung toàn bộ app) ──────
const otpSchema = new mongoose.Schema({
phone:     { type: String, required: true, index: true },
otp:       { type: String, required: true },
type:      { type: String, default: ‘auth’ }, // ‘auth’ | ‘shipper_register’ | ‘partner_gl’ | …
...
const slug = { gl: GiatLa, gv: GiupViec, cs: ChinaShop,
giat_la: GiatLa, giup_viec: GiupViec, china_shop: ChinaShop };
return slug[mod] || null;
}

function slugify(fe) {
return { gl: ‘giat_la’, gv: ‘giup_viec’, cs: ‘china_shop’ }[fe] || fe;
}
...
// Gửi SMS qua ESMS
async function sendSms(phone, message, requestId) {
const key = process.env.ESMS_API_KEY;
const sec = process.env.ESMS_SECRET_KEY;

// Dev mode: chỉ log, không gửi thật
if (!key || key === ‘your_esms_api_key’ || process.env.NODE_ENV !== ‘production’) {
console.log(`📱 [DEV-SMS] ${phone}: ${message}`);
return true;
}

const body = {
ApiKey: key, SecretKey: sec, Phone: phone, Content: message,
SmsType: ‘8’, IsUnicode: 0, Sandbox: 0,
RequestId: requestId || `CRB-${Date.now()}-${phone.slice(-4)}`,
};
const r = await axios.post(
‘https://rest.esms.vn/MainService.svc/json/SendMultipleMessage_V4_post_json/’,
body, { headers: { ‘Content-Type’: ‘application/json’ }, timeout: 10000 }
);
if (r.data.CodeResult !== ‘100’) throw new Error(’ESMS ’ + r.data.CodeResult);
return true;
}
...
// Form đăng ký (public)
app.get(’/shipper/register’, (req, res) => res.sendFile(path.join(__dirname, ‘public’, ‘shipper-register.html’)));
app.get(’/partner/register’, (req, res) => res.sendFile(path.join(__dirname, ‘public’, ‘partner-register.html’)));
...
await sendSms(phone,
  `[CRABOR] Ma xac minh cua ban la ${otp}. Co hieu luc 10 phut. Khong chia se ma nay.`,
  `CRB-OTP-${phone}-${Date.now()}`
);
...
```
const record = await OTP.findOne({ phone, type }).sort({ createdAt: -1 });
if (!record) return res.status(400).json({ success: false, message: 'OTP hết hạn hoặc chưa gửi' });
...
// SMS xác nhận
await sendSms(phone,
  `CRABOR: Ho so Shipper cua ban (${shipper.registerId}) da duoc tiep nhan. Chung toi se lien he trong 24-48h.`,
  `CRB-SP-${shipper.registerId}`
).catch(() => {});
...
const modName = { giat_la: 'Giặt Là', giup_viec: 'Giúp Việc', china_shop: 'China Shop' }[mod];

await sendSms(phone,
  `CRABOR: Ho so doi tac ${modName} (${partner.registerId}) da duoc tiep nhan. Chung toi se lien he trong 24-48h.`,
  `CRB-P-${partner.registerId}`
).catch(() => {});
...
```
const [totalS, totalGL, totalGV, totalCS,
       pendingS, pendingGL, pendingGV, pendingCS,
       todayS, todayGL, todayGV, todayCS,
...
res.json({ success: true, data: {
  total:   totalS + totalGL + totalGV + totalCS,
  shippers: totalS,
  partners: { gl: totalGL, gv: totalGV, cs: totalCS },
  pending: pendingS + pendingGL + pendingGV + pendingCS,
  ...
  todayRegistrations: todayS + todayGL + todayGV + todayCS,
...
? (type === ‘shipper’ ? [{ m: Shipper, t: ‘shipper’ }] : [{ m: getPartnerModel(type), t: type }])
: [{ m: Shipper, t: ‘shipper’ }, { m: GiatLa, t: ‘gl’ }, { m: GiupViec, t: ‘gv’ }, { m: ChinaShop, t: ‘cs’ }];
...
```
const Model = type === 'shipper' ? Shipper : getPartnerModel(type);
...
if (smsMap[status]) await sendSms(record.phone, smsMap[status], `CRB-NOTIF-${record.registerId}-${status}`).catch(()=>{});
