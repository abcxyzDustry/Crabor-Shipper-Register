...
app.get("/api/shipper/me", async (req, res) => {
  try {
    console.log('[GetMe] Session shipperId:', req.session?.shipperId);
    console.log('[GetMe] Session userPhone:', req.session?.userPhone);

    let shipper = null;
    if (req.session?.shipperId) {
      shipper = await Shipper.findById(req.session.shipperId);
    } else if (req.session?.userPhone) {
      shipper = await Shipper.findOne({ phone: req.session.userPhone });
    }

    if (!shipper) {
      return res.status(401).json({ success: false, message: "Chưa đăng nhập hoặc phiên hết hạn", notRegistered: true });
    }

    console.log('[GetMe] Found shipper:', shipper._id, shipper.fullName);
...
app.post("/api/shipper/check-account", async (req, res) => {
  try {
    ...
    console.log('[CheckAccount] Found shipper:', shipper._id, 'hasPassword:', !!shipper.password);
...
app.post("/api/shipper/login", async (req, res) => {
  try {
    ...
    const cookieHeader = res.getHeader("Set-Cookie");
    let cookieStr = Array.isArray(cookieHeader) ? cookieHeader[0]?.split(";")[0] : (cookieHeader||"").split(";")[0];
    if (!cookieStr && req.session.id) {
      cookieStr = `connect.sid=${req.session.id}; Path=/; HttpOnly; SameSite=Lax`;
    }
    console.log('[Login] Session created:', req.session.id, 'Cookie exists:', !!cookieStr);
    res.json({ success: true, shipper: { _id: shipper._id, fullName: shipper.fullName, phone: shipper.phone, status: shipper.status }, cookie: cookieStr || "", sessionId: req.session.id });
  } catch(err) {
...
app.post("/api/shipper/set-password", async (req, res) => {
  try {
    ...
    console.log('[SetPassword] Session created:', req.session.id, 'Cookie exists:', !!cookieStr);
    res.json({ success: true, shipper: { _id: shipper._id, fullName: shipper.fullName, phone: shipper.phone, status: shipper.status }, cookie: cookieStr || "", sessionId: req.session.id });
  } catch(err) {
