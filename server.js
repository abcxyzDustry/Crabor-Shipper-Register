...
app.post("/api/shipper/online", async (req, res) => {
  try {
    ...
    req.io.to("admin").emit("shipperOnline", { shipperId: req.session.shipperId, location: { lat, lng } });
    console.log('[Online] Shipper', req.session.shipperId, 'is now ONLINE at', lat, lng);
