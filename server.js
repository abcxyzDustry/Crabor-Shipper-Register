...
app.get("/api/admin/registrations/:type/:id", adminAuth, async (req, res) => {
  try {
    ...
    // Build documentsUrls: giữ Cloudinary URL + base64 (admin vẫn xem được), bỏ pending_upload
    const docsForAdmin = {};
    if (doc.documents) {
      Object.entries(doc.documents).forEach(([field, val]) => {
        if (val && val !== 'pending_upload') {
          // Giữ cả Cloudinary URL và base64 data URL
    ...
    // Avatar cũng xử lý tương tự
    if (doc.avatar && doc.avatar !== 'pending_upload' && !doc.avatar.startsWith('data:')) {
      docsForAdmin._avatar = doc.avatar;
    }
...
// POST /api/admin/registrations/:type/:id/upload-doc — Admin upload ảnh hồ sơ thay user
app.post("/api/admin/registrations/:type/:id/upload-doc", adminAuth, async (req, res) => {
  try {
    ...
    res.json({ success: true, url: secureUrl, field });
  } catch(err) {
