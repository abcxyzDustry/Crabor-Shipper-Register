...
app.use(session({
  ...
  cookie: {
    ...
    httpOnly: true,
    sameSite: 'none',       // cross-origin requests từ mobile
...
// GET /api/coco/status — public status cho widget
app.get("/api/coco/status", async (req, res) => {
  try {
    const backend = process.env.COCO_BRAIN || 'rule';
    const canReason = backend !== 'rule' && !!process.env.GROQ_API_KEY;
    res.json({ success:true, backend, canReason,
      note: canReason ? `✅ ${backend} — AI đang bật` : 'Rule-based mode. Set COCO_BRAIN=groq để bật AI.' });
  } catch(e) { res.status(500).json({ success:false, message: e.message }); }
});
...
app.post("/api/coco/chat", async (req, res) => {
  try {
    // Nhận cả text (field cũ) lẫn message (widget mới)
    const rawText = (req.body.text || req.body.message || '').trim();
    const text = rawText;
...
// ── POST /api/partner/check-account ──────────────────────────
// Kiểm tra tài khoản đã có mật khẩu chưa
app.post("/api/partner/check-account", async (req, res) => {
  try {
    const { phone, email } = req.body;
    if (!phone && !email) return res.status(400).json({ success: false, message: "Thiếu phone hoặc email" });
    const query = phone ? { phone: normalizePhone(phone) } : { email: email.toLowerCase().trim() };
    const models = [GiatLa, GiupViec, ChinaShop, FoodPartner, RideDriver];
    for (const Model of models) {
      const p = await Model.findOne(query).select("password status");
      if (p) {
        if (p.status === "rejected") return res.status(403).json({ success: false, message: "Tài khoản đã bị từ chối" });
        return res.json({ success: true, hasPassword: !!p.password });
      }
    }
...
// Đặt mật khẩu lần đầu → tạo session luôn
app.post("/api/partner/set-password", async (req, res) => {
  try {
    const { phone, email, password } = req.body;
    if (!password || password.length < 6) return res.status(400).json({ success: false, message: "Mật khẩu tối thiểu 6 ký tự" });
    const query = phone ? { phone: normalizePhone(phone) } : { email: email?.toLowerCase().trim() };
    const models = [
      { model: GiatLa, module: "giat_la" },
      { model: GiupViec, module: "giup_viec" },
    ...
    const hash = await bcrypt.hash(password, 10);
    for (const { model, module } of models) {
      const p = await model.findOneAndUpdate(query, { password: hash }, { new: true });
      if (p) {
...
app.post("/api/ride/book", async (req, res) => {
  try {
    ...
    // Lưu vào User.rideHistory hoặc collection riêng nếu có
    // Hiện tại trả về booking giả để app hoạt động
    const bookingId = "RIDE-" + Date.now().toString(36).toUpperCase();
    res.json({
      success: true,
      booking: {
        _id: bookingId, bookingId,
        userId: req.session.userId,
        vehicleType, fromAddress, toAddress,
...
app.patch("/api/ride/:id/cancel", async (req, res) => {
  try {
    if (!req.session.userId) return res.status(401).json({ success: false });
    res.json({ success: true, message: "Đã huỷ chuyến" });
  } catch(err) { res.status(500).json({ success: false, message: err.message }); }
});
...
app.post("/api/laundry/order", async (req, res) => {
  if (!req.session.userId) return res.status(401).json({ success: false });
  const orderId = "LAU-" + Date.now().toString(36).toUpperCase();
  res.status(201).json({ success: true, orderId, message: "Đặt lịch giặt thành công!" });
});
...
app.post("/api/laundry/orders/:id/rate", async (req, res) => {
  if (!req.session.userId) return res.status(401).json({ success: false });
  res.json({ success: true });
});
...
app.post("/api/cleaning/order", async (req, res) => {
  if (!req.session.userId) return res.status(401).json({ success: false });
  const orderId = "CLN-" + Date.now().toString(36).toUpperCase();
  res.status(201).json({ success: true, orderId, message: "Đặt lịch dọn dẹp thành công!" });
});
...
app.post("/api/shipper/location", async (req, res) => {
  try {
    const { lat, lng } = req.body;
    if (!req.session.userPhone) return res.status(401).json({ success: false });
    await Shipper.findOneAndUpdate({ phone: req.session.userPhone }, { lastLat: lat, lastLng: lng, lastLocationAt: new Date() });
    res.json({ success: true });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
});
...
app.post("/api/shipper/check-account", async (req, res) => {
  try {
    const { phone, email } = req.body;
    const query = phone ? { phone: normalizePhone(phone) } : { email: email?.toLowerCase().trim() };
    const shipper = await Shipper.findOne(query).select("_id hasPassword password");
    if (!shipper) return res.json({ success: true, exists: false });
    res.json({ success: true, exists: true, hasPassword: !!(shipper.password) });
  } catch(err) { res.status(500).json({ success: false, message: err.message }); }
});
...
app.post("/api/shipper/login", async (req, res) => {
  try {
    ...
    if (!shipper.password) return res.status(400).json({ success: false, message: "Chưa thiết lập mật khẩu. Vui lòng tạo mật khẩu mới." });
    const bcrypt  = require("bcryptjs");
    ...
    const cookieHeader = res.getHeader("Set-Cookie");
    const cookieStr = Array.isArray(cookieHeader) ? cookieHeader[0]?.split(";")[0] : (cookieHeader||"").split(";")[0];
    res.json({ success: true, shipper: { _id: shipper._id, fullName: shipper.fullName, phone: shipper.phone, status: shipper.status }, cookie: cookieStr });
  } catch(err) { res.status(500).json({ success: false, message: err.message }); }
});
...
app.post("/api/shipper/set-password", async (req, res) => {
  try {
    ...
    if (!shipper) return res.status(404).json({ success: false, message: "Tài khoản không tồn tại" });
    const bcrypt = require("bcryptjs");
    shipper.password = await bcrypt.hash(password, 10);
    await shipper.save();
    req.session.shipperId = shipper._id;
    req.session.userPhone = shipper.phone;
    req.session.role = "shipper";
    ...
    const cookieHeader = res.getHeader("Set-Cookie");
    const cookieStr = Array.isArray(cookieHeader) ? cookieHeader[0]?.split(";")[0] : (cookieHeader||"").split(";")[0];
    res.json({ success: true, shipper: { _id: shipper._id, fullName: shipper.fullName, phone: shipper.phone }, cookie: cookieStr });
  } catch(err) { res.status(500).json({ success: false, message: err.message }); }
});
...
app.post("/api/partner/check-account", async (req, res) => {
  try {
    ...
    for (const { model, module } of models) {
      const p = await model.findOne(query).select("_id password");
      if (p) return res.json({ success: true, exists: true, hasPassword: !!p.password, module });
    }
...
app.post("/api/partner/set-password", async (req, res) => {
  try {
    ...
    for (const model of pModels) {
      const p = await model.findOneAndUpdate(query, { password: hashed }, { new: true });
      if (p) {
        req.session.partnerId = p._id;
        req.session.userPhone = p.phone;
        req.session.role = "partner";
        ...
        const cookieHeader = res.getHeader("Set-Cookie");
        const cookieStr = Array.isArray(cookieHeader) ? cookieHeader[0]?.split(";")[0] : (cookieHeader||"").split(";")[0];
        await Storage?.setItem?.("crabor_partner", JSON.stringify({ _id: p._id, bizName: p.bizName, phone: p.phone }));
        return res.json({ success: true, partner: p, cookie: cookieStr, sessionId: req.session.id });
      }
    }
...
app.post("/api/payment/payos/create", async (req, res) => {
  try {
    if (!payOS) return res.status(503).json({ success: false, message: "PayOS chưa được cấu hình" });
...
app.get("/api/payment/payos/:orderCode", async (req, res) => {
  try {
    ...
    const info = await payOS.paymentRequests.getById({ id: req.params.orderCode });
...
async function findNearbyShippers(lat, lng, radiusKm = 5, limit = 5) {
  // Haversine filter từ DB rồi tính khoảng cách
  ...
  }).select("_id phone fullName location pushToken");
...
app.post("/api/order", async (req, res) => {
  try {
    ...
    const { partnerId, items, address, note, paymentMethod, voucherCode, shipFee } = req.body;
    if (!partnerId || !items?.length || !address)
      return res.status(400).json({ success: false, message: "Thiếu thông tin đơn hàng" });

    const user = await User.findById(req.session.userId).select("fullName phone");
    const FoodPartner = mongoose.models.FoodPartner;
    const partner = FoodPartner ? await FoodPartner.findById(partnerId).select("bizName address lastLat lastLng isAccepting") : null;

    if (partner && !partner.isAccepting)
    ...
    const serviceFee = Math.round(total * 0.05); // 5% phí dịch vụ
    ...
    // 1. Thông báo partner có đơn mới
    if (partnerId) {
    ...
    // 2. Thông báo admin
    req.io.to("admin").emit("newOrderNotification", { orderId: order.orderId, module: "food", total: order.finalTotal });
...
app.post("/api/shipper/online", async (req, res) => {
  try {
    ...
    const shipper = await Shipper.findByIdAndUpdate(
      ...
      { online: true, isAccepting: true, lastSeen: new Date() },
      { new: true }
    );
...
app.post("/api/shipper/location", async (req, res) => {
  try {
    ...
    await Shipper.findByIdAndUpdate(req.session.shipperId, { location: { lat, lng }, lastLocationAt: new Date() });
...
io.on("connection", (socket) => {
  // Chạy handler gốc
  if (originalHandler) originalHandler(socket);
...
// ── Coco AI v2 patch (Groq-powered /api/coco/chat + /api/nova/chat) ──
try { require('./server-coco-patch')(app, io); } catch(e) { console.log('[Coco Patch] Skip:', e.message); }
