...
app.get("/api/shipper/me", async (req, res) => {
  try {
    ...
    if (!shipper) {
      // Nếu session tồn tại nhưng không tìm được shipper → session cũ/invalid
      if (req.session?.shipperId || req.session?.userPhone) {
...
app.post("/api/shipper/login", async (req, res) => {
  try {
    ...
    // FIX: tự build cookie string từ session.id
    const sessionCookieValue = `connect.sid=s%3A${req.session.id}`;
...
app.post("/api/shipper/set-password", async (req, res) => {
  try {
    ...
    // FIX: express-session set Set-Cookie SAU res.end() nên getHeader() luôn rỗng
    // Tự build cookie string từ session.id để client lưu đúng
    const sessionCookieValue = `connect.sid=s%3A${req.session.id}`;
