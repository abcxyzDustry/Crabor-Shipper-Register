...
// POST /api/upload-doc — Upload ảnh hồ sơ lên Cloudinary, lưu URL vào MongoDB
// Body: { type, registerId, field, data (base64 data URL) }
app.post("/api/upload-doc", async (req, res) => {
  try {
    ...
    // Upload lên Cloudinary qua REST API (signed)
    const cloudName = process.env.CLOUDINARY_CLOUD_NAME;
    const apiKey    = process.env.CLOUDINARY_API_KEY;
    const apiSecret = process.env.CLOUDINARY_API_SECRET;

    const timestamp = Math.floor(Date.now() / 1000);
    const folder = "crabor_docs/" + type;

    // Cloudinary signed upload — SHA1, params sorted alphabetically (không kèm file/api_key)
    const sigStr = "folder=" + folder + "&timestamp=" + timestamp + apiSecret;
    const signature = crypto.createHash('sha1').update(sigStr).digest('hex');

    // Gửi lên Cloudinary bằng URLSearchParams
    ...
    const update = {};
    update["documents." + field] = secureUrl;

    const doc = await Model.findOneAndUpdate({ registerId }, { $set: update }, { new: false });
    if (!doc) return res.status(404).json({ success: false, message: "Không tìm thấy hồ sơ" });
...
app.get("/admin", (req, res) => {
  ...
  try {
    ...
    // Inject script hiển thị ảnh hồ sơ (nếu chưa có)
    if (!html.includes("CRABOR_DOC_PATCH")) {
      const patch = `
<style id="CRABOR_DOC_PATCH">
.doc-imgs-grid{display:flex;flex-wrap:wrap;gap:10px;margin-top:10px}
.doc-img-box{position:relative;cursor:pointer}
.doc-img-box img{width:120px;height:90px;object-fit:cover;border-radius:8px;border:2px solid #e2e8f0;background:#f7fafc}
.doc-img-box .doc-lbl{font-size:10px;color:#718096;text-align:center;margin-top:3px}
.doc-img-box img:hover{border-color:#E8504A;transform:scale(1.05);transition:all .2s}
#docLightbox{display:none;position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:9999;align-items:center;justify-content:center;flex-direction:column;gap:12px}
#docLightbox img{max-width:92vw;max-height:82vh;border-radius:12px;box-shadow:0 8px 32px rgba(0,0,0,.5)}
#docLightbox .lb-lbl{color:#fff;font-size:15px;font-weight:700}
#docLightbox .lb-close{position:absolute;top:16px;right:20px;color:#fff;font-size:32px;cursor:pointer;line-height:1}
.doc-missing{width:120px;height:90px;border-radius:8px;border:2px dashed #CBD5E0;display:flex;align-items:center;justify-content:center;font-size:22px;background:#f7fafc;color:#A0AEC0}
</style>
<div id="docLightbox" onclick="closeLb(event)">
  <span class="lb-close" onclick="document.getElementById('docLightbox').style.display='none'">✕</span>
  <div class="lb-lbl" id="lbLabel"></div>
  <img id="lbImg" src="">
</div>
<script>
function closeLb(e){if(e.target===document.getElementById('docLightbox'))document.getElementById('docLightbox').style.display='none';}
...
// Hàm render ảnh hồ sơ - gọi sau khi render card hồ sơ
function renderDocImages(container, docsUrls, type, registerId) {
  if(!docsUrls) return;
  const labels={cccdFront:'CCCD trước',cccdBack:'CCCD sau',selfie:'Selfie',shopFront:'Mặt tiền',shopInside:'Bên trong',vehicleImg:'Phương tiện',productSample:'Sản phẩm mẫu',importDoc:'Giấy tờ nhập',licenseImg:'Bằng lái',driverLicense:'Bằng lái',vehicleReg:'Đăng ký xe'};
  const hasAny = Object.values(docsUrls).some(v=>v);
  if(!hasAny) return;
  const grid = document.createElement('div');
  grid.className='doc-imgs-grid';
  grid.innerHTML='<div style="width:100%;font-size:12px;font-weight:700;color:#4A5568;margin-bottom:2px">📎 Hồ sơ ảnh</div>';
  Object.entries(docsUrls).forEach(([field,url])=>{
    const box=document.createElement('div');
    box.className='doc-img-box';
    const lbl=labels[field]||field;
    if(url){
      box.innerHTML='<img src="'+url+'" alt="'+lbl+'" loading="lazy" onclick="openLb(this.src,\''+lbl+'\')"><div class="doc-lbl">'+lbl+'</div>';
    } else {
      box.innerHTML='<div class="doc-missing">🖼️</div><div class="doc-lbl" style="color:#CBD5E0">'+lbl+' (chưa có)</div>';
    }
    grid.appendChild(box);
  });
  container.appendChild(grid);
}
// Monkey-patch: sau mỗi lần render card hồ sơ, tự động inject ảnh
// Tìm tất cả card hồ sơ và gọi API lấy documentsUrls
async function injectDocImages() {
  // Tìm các element có data-regid (admin.html cần set attr này khi render card)
  document.querySelectorAll('[data-regid][data-regtype]').forEach(async el=>{
    if(el.dataset.docsLoaded) return;
    el.dataset.docsLoaded='1';
    const rid=el.dataset.regid, rtype=el.dataset.regtype;
    try{
      const r=await fetch('/api/admin/registrations/'+rtype+'/'+el.dataset.mongoId,{credentials:'include'});
      const d=await r.json();
      if(d.success && d.data.documentsUrls){
        let target=el.querySelector('.doc-imgs-target')||el;
        renderDocImages(target,d.data.documentsUrls,rtype,rid);
      }
    }catch(_){}
  });
}
// Observe DOM changes để inject ảnh tự động khi admin load danh sách
const _docObs=new MutationObserver(()=>{ clearTimeout(_docObs._t); _docObs._t=setTimeout(injectDocImages,400); });
_docObs.observe(document.body,{childList:true,subtree:true});
setTimeout(injectDocImages,1000);
</script>`;
...
app.get("/api/orders/:id", async (req, res) => {
  try {
    ...
    if (order.shipperId) {
      try {
        const sh = await Shipper.findById(order.shipperId).select("fullName phone vehiclePlate avatar documents").lean();
        if (sh) {
          shipperInfo = {
            _id: sh._id,
...
// GET /api/shipper/cash-stats — Thống kê tiền mặt shipper cần nộp CRABOR (15%)
...
try {
  ...
  console.log("[OK] PayOS initialized, methods:", typeof payOS.createPaymentLink, typeof payOS.paymentRequests?.create);
} catch(e) {
...
app.get("/api/admin/registrations/:type/:id", adminAuth, async (req, res) => {
  try {
    ...
    // documents chỉ lưu Cloudinary URL → trả thẳng
    const docsForAdmin = {};
    if (doc.documents) {
      Object.entries(doc.documents).forEach(([field, val]) => {
        if (val && val !== 'pending_upload' && !val.startsWith('data:')) {
          docsForAdmin[field] = val; // Cloudinary URL
        } else {
...
async function calcEarnings(order) {
  ...
  // Shipper nhận 100% phí ship (không thay đổi)
  ...
  try {
    ...
    const payload = {
      ...
      order: {
        ...
        pickupLng,
        deliveryAddress: order.address,
        deliveryLat: order.deliveryLat || order.lat,
        deliveryLng: order.deliveryLng || order.lng,
        note: order.note,
        customerName: order.customerName,
        customerPhone: order.customerPhone,
        module: order.module,
