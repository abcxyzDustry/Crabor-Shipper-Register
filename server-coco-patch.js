...
module.exports = function installCocoAI(app, io) {
  ...
  app.use('/api/coco/chat', async (req, res, next) => {
    ...
    try {
      ...
      // Build user context từ session (giống route cũ nhưng đầy đủ hơn)
      let userCtx = {};
      const userId = req.session?.userId;

      if (userId) {
        try {
          // Dùng mongoose model đã có trong server.js (global scope)
          const User  = require('mongoose').model('User');
          const Order = require('mongoose').model('Order');

          const [user, recentOrders] = await Promise.all([
            User.findById(userId).select('fullName phone totalSpent loyaltyPts walletBalance'),
            Order.find({ customerId: userId, status: { $ne: 'cancelled' } })
              .sort({ createdAt: -1 }).limit(3).select('status orderId total module'),
          ]);
          ...
          if (user) {
            ...
            // Check BNPL nếu có model
            try {
              const BNPLInvoice = require('mongoose').models.BNPLInvoice;
              if (BNPLInvoice) {
                const unpaid = await BNPLInvoice.find({
                  userId, status: 'unpaid'
                }).select('amount dueDate').limit(3);
                if (unpaid.length) userCtx.unpaidInvoices = unpaid;
              }
            } catch(_) {}
          }
        } catch(e) {
          console.error('[Coco Patch] userCtx error:', e.message);
        }
      }
      ...
      const result = await CocoReasoning.answer(userInput, history, userCtx, sid);
  ...
  // ════════════════════════════════════════════════════════════
  //  ENHANCED: GET /api/coco/brain/status — Chi tiết hơn
  // ════════════════════════════════════════════════════════════
  ...
  console.log('[Coco AI Patch] ✅ Installed — /api/coco/* upgraded to Groq-powered reasoning');
